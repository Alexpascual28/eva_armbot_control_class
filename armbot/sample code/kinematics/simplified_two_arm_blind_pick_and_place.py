...
def initialize_arm():
    ...
    arm_hostname = "evatrendylimashifterpt410" #"evacunningyorkartistpt410" # "evatrendylimashifterpt410"
    arm_ip = "144.32.152.105" #"144.32.152.2" # "144.32.152.105"
    token = "1462980d67d58cb7eaabe8790d866609eb97fd2c" #"4b1c26278a566e0419165a3acd025dd83d32b160" # "1462980d67d58cb7eaabe8790d866609eb97fd2c"
...
def main():
    ...
    # Start block
    try:     
        home_robot(kn, eva); # from [0, -0.2, 0.3]
        
        test_x_axis_shift(kn, eva); # range: 0.3 (right) to -0.3 (left) abs: ([-0.3, -0.2, 0.3], [0.3, -0.2, 0.3]) left-right
        home_robot(kn, eva);
        
        test_y_axis_shift(kn, eva); # range: 0.1 (in) to -0.3 (out) abs: ([0, -0.5, 0.3], [0, -0.1, 0.3]) out-in
        home_robot(kn, eva);
        
        test_z_axis_shift(kn, eva); # range: 0.5 (up) to -0.12 (down) abs: ([0, -0.2, 0.18], [0, -0.2, 0.5]) down-up
        home_robot(kn, eva);

        #move_end_efector(kn, eva, [-0.3, 0, 0.18])

        #for y in range(-6, 3):
            #y = (y/2) * 0.1;

            #move_end_efector(kn, eva, [-0.3, y, 0.18])
            #move_end_efector(kn, eva, [-0.3, y, 0.5])
            #move_end_efector(kn, eva, [0.3, y, 0.5])
            #move_end_efector(kn, eva, [0.3, y, 0.18])

        #move_end_efector(kn, eva, [-0.3, -0.1, 0.15])
        home_robot(kn, eva);
