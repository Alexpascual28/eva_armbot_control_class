...
def initialize_arm():
    ...
    arm1_hostname = "evatrendylimashifterpt410" #"evacunningyorkartistpt410" # "evatrendylimashifterpt410"
    arm1_ip = "144.32.152.105" #"144.32.152.2" # "144.32.152.105"
    token1 = "1462980d67d58cb7eaabe8790d866609eb97fd2c" #"4b1c26278a566e0419165a3acd025dd83d32b160" # "1462980d67d58cb7eaabe8790d866609eb97fd2c"

    arm2_hostname = "evacunningyorkartistpt410" #"evacunningyorkartistpt410" # "evatrendylimashifterpt410"
    arm2_ip = "144.32.152.2" #"144.32.152.2" # "144.32.152.105"
    token2 = "4b1c26278a566e0419165a3acd025dd83d32b160" #"4b1c26278a566e0419165a3acd025dd83d32b160" # "1462980d67d58cb7eaabe8790d866609eb97fd2c"
  
    # Create an “eva” object with these parameters to connect to the arm itself on the network
    eva1 = Eva(arm1_ip, token1)
    eva2 = Eva(arm2_ip, token2)
    print("Done.\n")
    return eva1, eva2
...
def main():
    ...
    eva1, eva2 = initialize_arm();
    
    # Start block
    try:     
        home_robot(kn, eva1); # from [0, -0.2, 0.3]
        home_robot(kn, eva2); # from [0, -0.2, 0.3]

        open_gripper(eva1)
        open_gripper(eva2)

        move_end_efector(kn, eva1, [0, -0.2, 0.18])
        move_end_efector(kn, eva2, [0, -0.2, 0.18])
        ...
        move_end_efector(kn, eva1, [0.2, -0.2, 0.22])
        move_end_efector(kn, eva2, [-0.2, -0.2, 0.22])
...
def move_end_efector(kn, eva, absolute_position):
    current_pose = kn.calculate_joint_angles(absolute_position, position_type = "absolute")
    with eva.lock():
        eva.control_wait_for_ready()
        eva.control_go_to(current_pose)

def shift_end_efector(kn, eva, relative_position):
    current_pose = kn.calculate_joint_angles(relative_position, position_type = "relative")
    with eva.lock():
        eva.control_wait_for_ready()
        eva.control_go_to(current_pose)

def open_gripper(eva):
...
def test_x_axis_shift(kn, eva):
    print("Testing X axis increments...");

    for x in range(6):
        target_position = [0.05, 0, 0]
...
def test_y_axis_shift(kn, eva):
    print("Testing X axis increments...");

    for x in range(2):
        target_position = [0, 0.05, 0]
